    ...
    <div>
        ...
        <v-data-table dense :headers="headers" :items="items" sort-by="name" class="elevation-1 ma-4" hide-default-footer disable-pagination :loading="loading" :key="key">
            ...
            <template v-slot:[`item.a`]="{ item }">
                <div v-html="get_from_catalog(item.a, 'products', 'name' )"></div>
            </template> 
            <template v-slot:[`item.b`]="{ item }">
                <div v-html="get_from_catalog(item.b, 'products', 'name' )"></div>
            </template>     
        ...
        <v-card class="pa-6">
            ...
            <v-autocomplete :items="$store.state.catalogs.products" v-model="product_a" :label="$t('Select a product')" item-text="name" item-value="url" :rules="RulesSelection(true)"></v-autocomplete>
            <v-autocomplete :items="$store.state.catalogs.products" v-model="product_b" :label="$t('Select a product')" item-text="name" item-value="url" :rules="RulesSelection(true)"></v-autocomplete>
            <v-card-actions>
