<template>
    <div class="pa-6">
        <h1>{{ f($t("Quotes massive update of '[0]'"), [product.fullname]) }}</h1>
        <v-card class="pa-5">
            <v-form ref="form">
                <v-file-input 
                    name="csv_file1" 
        ...
        <v-data-table density="compact" :headers="headers" :items="items" :sort-by="[{key:'product',order:'asc'}]" class="elevation-1 pt-6" :loading="loading" :items-per-page="10000">
    ...
    export default {
        ...
        methods: {
            ...
            async submmit() {
                ...
                let readed = {}
                if (this.filename[0]) {
                    readed = await this.readDocument(this.filename[0])
            ...
            isButtonDisabled() {
                ...
                if (!this.filename || this.filename.length === 0) return true
                return false
            }
        }
    }
