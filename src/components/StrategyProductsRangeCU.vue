    ...
    <div>
        ...
        <v-card class="pa-8 mt-2">
            <v-form ref="form" v-model="form_valid">
                <v-text-field data-test="StrategyProductsRangeCU_Name" :readonly="mode == 'D'"
                ...
                <v-select data-test="StrategyProductsRangeCU_Type" readonly
                    :items="getArrayFromMap(useStore().strategiestypes)" v-model="new_strategy.strategy.type"
                    :label="$t('Select strategy type')" item-title="name" item-value="id"
                ...
                <v-row class="d-flex flex-row mx-auto" justify="center">
                    <v-text-field class="mr-4" data-test="StrategyProductsRangeCU_PercentageBetweenRanges"
                        ...
                        :label="$t('Set percentage between ranges (0.05 for example)')"
                        :placeholder="$t('Set percentage between ranges (0.05 for example)')"
                        :rules="RulesFloat(10, false, 6)" counter="10" />
                    <v-text-field class="mr-4" data-test="StrategyProductsRangeCU_PercentageGains" :readonly="mode == 'D'"
                        v-model.number="new_strategy.percentage_gains"
                        :label="$t('Set percentage gains (0.1 for example)')"
                        :placeholder="$t('Set percentage between ranges (0.1 for example)')"
                        :rules="RulesFloat(10, false, 6)" counter="10" />
                    <v-text-field class="mr-4" data-test="StrategyProductsRangeCU_Amount" :readonly="mode == 'D'"
                        v-model.number="new_strategy.amount" :label="$t('Set range amount')"
                        :placeholder="$t('Set range amount')" :rules="RulesFloat(10, false, 6)" counter="10" />
                </v-row>

                <v-row class="d-flex flex-row mx-auto" justify="center">
                    <v-select data-test="StrategyProductsRangeCU_RecomendationMethod" class="mr-5"
                        :items="getArrayFromMap(useStore().recomendation_methods)"
                        v-model="new_strategy.recomendation_method" item-title="name" item-value="id"
                        :rules="RulesSelection(true)" :label="$t('Select a recomendation method')" />
    ...
    methods: {
        ...
        async accept() {
            ...
            if (this.mode == "U") {
                ...
                axios.post(`${this.useStore().apiroot}/api/strategies_productsrange/`, this.new_strategy)
                    .then(() => {
                        this.$emit("cruded")
                    })
            } else if (this.mode == "D") {
                if (await this.myConfirm(this.$t("This products range strategy will be deleted. Do you want to continue?")) == false) {
                    return
                }
