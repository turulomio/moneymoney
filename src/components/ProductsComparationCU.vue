    ...
    <div>    
        ...
        <v-card class="pa-8 mt-2">
            <v-form ref="form" v-model="form_valid" lazy-validation>
                ...
                <v-autocomplete :items="$store.state.catalogs.products" v-model="newpc.a" :label="$t('Select product A (stronger)')" item-text="name" item-value="url" :rules="RulesSelection(true)"></v-autocomplete>
                <v-autocomplete :items="$store.state.catalogs.products" v-model="newpc.b" :label="$t('Select product B')" item-text="name" item-value="url" :rules="RulesSelection(true)"></v-autocomplete>
            </v-form>
            <v-card-actions>
                <v-spacer></v-spacer>
