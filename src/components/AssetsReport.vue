...
<script>
    ...
    import {convertImageToDataURL} from "@/functions"
    ...
    export default {
        ...
        methods:{
            ...
            async launch_report_alternative(){
                ...
                const docDefinition = {
                    content: [    
                        { image: await convertImageToDataURL(imgMoneymoney), width: 200, alignment: 'center', margin: [0, 125, 0, 0] },
            ...
            report_assets(){
                ...
                if (this.results.balance-this.results.last_year_balance>=0){
                    r.push({text: f(this.$t("At the end of last year, you had [0]. Currently, you have [1], which meas you have gained [2]."), [
                        ...
                        this.percentage_html((this.results.balance-this.results.last_year_balance)/this.results.last_year_balance)
                    ]) , style:"body"})
            ...
            pdfmake_loo_to_table(loo){
                var headers=[]
                for (const key in loo[0]){
                    headers.push(key)
                }
                console.log("HEADERSA", headers)
                var body=[]
                // Adding headers
                var row=[]
                
                headers.forEach(header=>{
                    row.push({text:header,style:"tableHeader",fillColor:'#cccccc'})
                })
                body.push(row)
                loo.forEach(o => {
                    console.log(o)
                ...
                //                 widths: ['*', 'auto', 100, '*'],
                //                 body: [                [
                //                         { text: 'First', style: 'tableHeader', fillColor: '#cccccc' },
                //                         { text: 'Second', style: 'tableHeader', fillColor: '#cccccc' },
                //                         { text: 'Third', style: 'tableHeader', fillColor: '#cccccc' },
                //                         { text: 'The last one', style: 'tableHeader', fillColor: '#cccccc' }
                //                     ],
            ...
            report_assets_by_bank(){
                var r=[]
                r.push(this.pdfmake_loo_to_table(this.results.assets_by_bank))
                return r
            },
            ...
            get_report_data(){
                ...
                axios.all([
                    axios.get(`${this.useStore().apiroot}/api/banks/withbalance/`, this.myheaders()),
                    axios.get(`${this.useStore().apiroot}/reports/annual/${year}/`, this.myheaders()),
                    axios.get(`${this.useStore().apiroot}/reports/annual/income/${year}/`, this.myheaders()),
                    axios.get(`${this.useStore().apiroot}/reports/annual/gainsbyproductstypes/${year}/`, this.myheaders()),
