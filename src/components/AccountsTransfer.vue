    ...
    <div>    
        ...
        <v-card class="pa-8 mt-2">
            <v-form ref="form" v-model="form_valid" lazy-validation>
                ...
                <v-autocomplete :items="$store.state.accounts.filter(v =>v.active==true)" v-model="transfer.origin" :label="$t('Select an origin account')" item-text="name" item-value="url" required :rules="RulesSelection(true)"></v-autocomplete>
                <v-autocomplete :items="$store.state.accounts.filter(v =>v.active==true)" v-model="transfer.destiny" :label="$t('Select a destiny account')" item-text="name" item-value="url" required :rules="RulesSelection(true)"></v-autocomplete>
