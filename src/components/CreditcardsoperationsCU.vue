    ...
    <div>    
        ...
        <v-card class="pa-6">
            <v-form ref="form_cco" v-model="form_valid_cco" lazy-validation>
                ...
                <v-autocomplete :items="$store.state.concepts" v-model="newcco.concepts" :label="$t('Select a concept')" item-text="name" item-value="url" :rules="RulesSelection(true)"></v-autocomplete>
                <v-text-field v-model="newcco.amount" type="number" :label="$t('Operation amount')" :placeholder="$t('Operation amount')" :rules="RulesFloat(30,true)" counter="30"/>
                <v-text-field v-model="newcco.comment" type="text" :label="$t('Operation comment')" :placeholder="$t('Operation comment')" :rules="RulesString(200, false)" counter="200"/>
            </v-form>
            <v-card-actions>
                <v-spacer></v-spacer>
