...
<!--                     ACCOUNTS -->
                    <v-list-item link router :to="{ name: 'accounts_list'}" v-if="useStore().logged" prepend-icon="mdi-calculator" :title="$t('Accounts')" />
<!--                     INVESTMENTS -->
                    <v-list-item link router :to="{ name: 'investments_list'}" v-if="useStore().logged" prepend-icon="mdi-finance" :title="$t('Investments')" />

<!--                     ORDERS -->
                    <v-list-item link router :to="{ name: 'orders_list'}" v-if="useStore().logged" prepend-icon="mdi-cart" :title="$t('Orders')" />

<!--                     PRODUCTS -->
                    <v-list-group value="Products" v-if="useStore().logged">
...
<!--                     REPORTS -->
                    <v-list-group value="Reports" v-if="useStore().logged">
                        ...
                        <v-list-item link router :to="{ name: 'reports_zero_risk'}">
                            <v-list-item-title>{{ $t("Zero risk investments") }}</v-list-item-title>
                        </v-list-item>
                    </v-list-group>
            
<!--                     STRATEGIES -->
                    <v-list-item link router :to="{ name: 'strategies_list'}" v-if="useStore().logged" prepend-icon="mdi-strategy" :title="$t('Strategies')" />
<!--                     ADMINISTRATION -->
...
<!--                     HELP -->
                    <v-list-group value="Help">
                        ...
                        <v-list-item link router :to="{ name: 'statistics'}" :title="$t('Statistics')" v-if="useStore().logged"/>
        ...
        <v-app-bar color="primary" dark  fixed fill-height app >
            ...
            <v-btn :to="{ name: 'assetsreport'}" v-if="useStore().logged"><v-icon icon="mdi-book" dark></v-icon></v-btn>
            <v-spacer />
            <h1 class="font-weight-black text-no-wrap text-truncate" >{{ $t("Money Money. Another way to manage your finances") }}</h1>
            <v-spacer />            
            <v-btn :to="{ name: 'currencies'}" v-if="useStore().logged"><v-icon icon="mdi-currency-eur" dark></v-icon></v-btn>
            <BtnSwitchLanguages />
            <BtnLogIn v-show="!useStore().logged" class="mr-4"/>
            <BtnLogOut v-show="useStore().logged" class="mr-4"/>
